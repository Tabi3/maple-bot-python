...
class View(nextcord.ui.View):
    def __init__(self, *, timeout: float = 180, auto_defer: bool = True, item=None):
...
class Rule34Select(nextcord.ui.Select):
    ...
    async def callback(self, interaction: nextcord.Interaction, view=None):
        ...
        ((_view := {"view": view or self.myview, "ephemeral": True}).update({"embed": embed} if rule34[1] .split('?')[
         0].split('.')[-1].lower() not in "mp4 mov wmv avi mkv" else {"embed": embed, "file": nextcord.File(fp=fp)}))
...
class HelpSelect(nextcord.ui.Select):
    def __init__(self, client) -> None:
        ...
        super().__init__(placeholder="Select a module", options=[
            ...
            for cog in cogs if len(cogs[cog].__cog_application_commands__) or len(cogs[cog].__cog_commands__)
        ])

    async def callback(self, interaction: nextcord.Interaction):
        cogs, cog, prefix = self.bot_cogs, self.values[0], self.client.command_prefix(..., interaction)
        slash_commands, prefix_commands = cogs[cog].__cog_application_commands__, cogs[cog].__cog_commands__
        print(cogs)
        embed = nextcord.Embed(
            title=cog, description=cogs[cog].__doc__ or "No description provided", color=0xF1828D
            ).add_field(name='Prefixed Commands', inline=False, value = "These commands require a prefix"
            ).add_field(name='Command names', value='`' + ('\n'.join(i.name for i in prefix_commands) or '...') + '`'
            ).add_field(name='Command Syntax', value='`' + ('\n'.join(' '.join(f"<{i.split(',')[0]}>" 
                                                                             for i in command.__dict__['params']
                                                                             if i not in "self _ ctx ") 
                                                                    or '<...>' for command in prefix_commands) or '...') + '`'
            ).add_field(name='Command Description', value='`' + ('\n'.join(i.help or "..." for i in prefix_commands) or '...') + '`'
            ).add_field(name='Slashed Commands', inline=False, value = "These commands are slash commands"
            ).add_field(name='Command names', value='`' + ('\n'.join(i.name for i in slash_commands or '...') + '`')
            ).add_field(name='Command Syntax', value='`' + ('\n'.join(' '.join(f'<{i}>'
                                                                               for i in command.options)
                                                                      or '<...>' for command in slash_commands) or '...') + '`'
            ).add_field(name='Command Description', value='`' + ('\n'.join(i._description or "..." for i in slash_commands) + '`')
            ) if cog != 'Help' else nextcord.Embed(
            title="Bot Help", description="Hello there, I'm Maple, developed by Tafí🍃#1290 in 2022 Around April.\n"
